...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `stock_total_1d_view` AS select `trade`.`RIC` AS `RIC`,`trade`.`Ticker` AS `Ticker`,date_format(`trade`.`date`,'%Y-%m-%d') AS `dat`,`rate`.`rate_id` AS `rate_id`,sum((case when (`trade`.`client_side` = 'buy') then `trade`.`size` else 0 end)) AS `total_buy_size`,sum((case when (`trade`.`client_side` = 'sell') then `trade`.`size` else 0 end)) AS `total_sell_size` from ((`trade` join `stock` on(((`trade`.`RIC` = `stock`.`RIC`) and (`trade`.`Ticker` = `stock`.`Ticker`)))) join `rate` on((`trade`.`rate_id` = `rate`.`rate_id`))) group by `trade`.`RIC`,`trade`.`Ticker`,date_format(`trade`.`date`,'%Y-%m-%d');
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `stock_total_ytd_view` AS select `trade`.`RIC` AS `RIC`,`trade`.`Ticker` AS `Ticker`,`rate`.`rate_id` AS `rate_id`,sum((case when (`trade`.`client_side` = 'buy') then `trade`.`size` else 0 end)) AS `total_buy_size`,sum((case when (`trade`.`client_side` = 'sell') then `trade`.`size` else 0 end)) AS `total_sell_size` from ((`trade` join `stock` on(((`trade`.`RIC` = `stock`.`RIC`) and (`trade`.`Ticker` = `stock`.`Ticker`)))) join `rate` on((`trade`.`rate_id` = `rate`.`rate_id`))) where (year(`trade`.`date`) = year(curdate())) group by `trade`.`RIC`,`trade`.`Ticker`;

-- ----------------------------
-- View structure for total_view
-- ----------------------------
DROP VIEW IF EXISTS `total_view`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `total_view` AS select sum(`stock_total_1d_view`.`total_buy_size`) AS `total_buy`,sum(`stock_total_1d_view`.`total_sell_size`) AS `total_sell`,(sum(`stock_total_1d_view`.`total_buy_size`) - sum(`stock_total_1d_view`.`total_sell_size`)) AS `quantity`,sum(((`stock_total_1d_view`.`total_buy_size` * `stock`.`price`) * `rate`.`rate`)) AS `total_buy_notional`,sum(((`stock_total_1d_view`.`total_sell_size` * `stock`.`price`) * `rate`.`rate`)) AS `total_sell_notional`,(sum(((`stock_total_1d_view`.`total_buy_size` * `stock`.`price`) * `rate`.`rate`)) - sum(((`stock_total_1d_view`.`total_sell_size` * `stock`.`price`) * `rate`.`rate`))) AS `quantity_notional` from ((`stock_total_1d_view` join `stock`) join `rate`) where ((`stock_total_1d_view`.`RIC` = `stock`.`RIC`) and (`stock_total_1d_view`.`Ticker` = `stock`.`Ticker`) and (`stock_total_1d_view`.`rate_id` = `rate`.`rate_id`));
